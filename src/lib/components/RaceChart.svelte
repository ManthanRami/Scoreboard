<script lang="ts">
  ...
  const leaderIndex = $derived(sortedPlayers.findIndex((player: RacePlayer) =>
    isLowScoreBetter ? player.totalScore === Math.min(...players.map((p: RacePlayer) => p.totalScore)) : player.totalScore === maxScore
  ));
</script>
    ...
    <div class="space-y-1">
      ...
      <div class="h-3 w-full bg-surface-variant rounded-full overflow-hidden border border-border/50">
        <div
          ...
          class:bg-gold={i === leaderIndex}
          class:bg-primary={i !== leaderIndex}
